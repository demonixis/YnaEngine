...
namespace Yna.Samples
{
    ...
    public class Menu : YnState2D
    {
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            if (YnG.Touch.Tapped)
            {
                Point touch = YnG.Touch.GetPosition(0).ToPoint();

                foreach (YnWidget widget in Gui[2].Children)
                {
                    if (widget is YnTextButton)
                    {
                        YnButton button = widget as YnButton;
                        Rectangle rect = new Rectangle(button.Parent.X + button.X, button.Parent.Y + button.Y, button.Width, button.Height);
                        if (rect.Contains(touch))
                        {
                            YnG.StateManager.SetStateActive(button.Name, true);
                        }
                    }
                }
            }
